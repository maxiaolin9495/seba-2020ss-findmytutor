...
import { WEEKS_PER_TIMESPAN, DAYS_IN_WEEK } from './AvailableTimesComponents/Constants';

import CalendarSelector from './AvailableTimesComponents/CalendarSelector';
import EventsStore from './AvailableTimesComponents/EventsStore';
import Slider from './AvailableTimesComponents/Slider';
import Week from './AvailableTimesComponents/Week';
import HelpFunctionCollections from './AvailableTimesComponents/HelpFunctionCollections';
...
export default class AvailableTimes extends React.Component {
    constructor({
        ...
        const normalizedSelections = recurring ?
            HelpFunctionCollections.normalizeRecurringSelections(initialSelections, timeZone, weekStartsOn) :
            initialSelections;
        this.state = {
            weeks: [],
            currentWeekIndex: 0,
        ...
        normalizedSelections.forEach((selection) => {
            const week = HelpFunctionCollections.weekAt(weekStartsOn, selection.start, timeZone);
        ...
        if (onChange) {
            if (recurring) {
                ...
                onChange(startingFirstWeek.map(selection =>
                    HelpFunctionCollections.makeRecurring(selection, timeZone, weekStartsOn)));
            } else {
    ...
    expandWeeks(weeks, weekIndex) {
        ...
        while (addedWeeks < WEEKS_PER_TIMESPAN) {
            const week = newWeeks.length ?
                HelpFunctionCollections.weekAt(weekStartsOn,
                    oneWeekAhead(newWeeks[newWeeks.length - 1].days[3].date, timeZone), timeZone)
                : HelpFunctionCollections.weekAt(weekStartsOn, new Date(), timeZone);
...
AvailableTimes.propTypes = {
    ...
    availableDays: PropTypes.arrayOf(HelpFunctionCollections.validateDays),
