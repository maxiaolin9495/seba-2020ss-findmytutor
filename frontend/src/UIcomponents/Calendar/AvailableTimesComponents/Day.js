...
export default class Day extends PureComponent {
    ...
    handleMouseDown(e) {
        ...
        const dateAtPosition = HelpFunctionCollections.toDate(this.props.date, position, timeZone);
    ...
    handleMouseMove({ pageY }) {
        ...
        this.setState(({ minLengthInMinutes, selections, edge, index, lastKnownPosition, target }) => {
            ...
            if (edge === 'both') {
                // move element
                const diff = HelpFunctionCollections.toDate(date, position, timeZone).getTime() -
                    HelpFunctionCollections.toDate(date, lastKnownPosition, timeZone).getTime();
                let newStart = new Date(selection.start.getTime() + diff);
                let newEnd = new Date(selection.end.getTime() + diff);
                if (HelpFunctionCollections.hasOverlap(selections, newStart, newEnd, index)) {
                    return {};
                }
...
//                        toast.error('The minimal time span must larget than an hour');
                ...
                const newEnd = HelpFunctionCollections.toDate(date, Math.max(minPos, position), timeZone);
                if (HelpFunctionCollections.hasOverlap(selections, selection.start, newEnd, index)) {
                    // Collision! Let
                    return {};
                }
    ...
    render() {
        ...
        if (HelpFunctionCollections.inSameDay(date, new Date(), timeZone)) {
